...
The frontend and `/tryon` API can override output resolution, inference steps, seed mode, deterministic best-effort mode, and intermediate artifact saving per job, so fast preview runs can use lower resolution or fewer steps without changing YAML config. The frontend runs jobs asynchronously by default, polls status every 500 ms, keeps the category control as a compact dropdown, and shows uploaded person/garment previews inside the upload tiles instead of a separate preview grid. Garment upload slots are filtered by category: underwear categories show only the relevant underwear slot, bra shows only bra/top, dress shows only dress, and full outfit keeps all garment slots available. The person upload tile reserves more space for filename/status text, while garment upload tiles stay horizontal and reserve a larger preview strip for the product image. The result panel keeps the artifact manifest and debug preview artifacts collapsed by default, but visible mask previews are shown in their own open section whenever the backend returns mask URLs; `Debug` is enabled by default for new UI sessions so mask, overlay, and crop artifacts are saved unless the user disables it. History refreshes again when a job reaches a terminal status, records seed/config/output/timing, and can be filtered by `All`, `Man`, `Woman`, plus a `Success only` toggle that hides failed or cancelled jobs. Each history filter asks the backend for its own latest 100 matching jobs, so `Man` and `Woman` are not filtered from the already-limited `All` list. Job status includes per-stage timing for queue, running/preprocess, generation, refinement, and completion; while a job is active, the toolbar, generate button, and running timeline stage show a live elapsed-second counter based on the browser clock, and skipped refinement is shown explicitly when FLUX refine is disabled or unavailable. Generated core images are composited back onto the original person with the soft garment mask before final output, so global engines such as Klein cannot change unmasked pixels; the raw model image is still saved as `core_output_raw.png`. The engine dropdown includes `Klein LoRA bnb 4-bit`, which still loads the Try-On LoRA and only forces local Klein onto `device_map=cuda` with `bnb_4bit` transformer/text-encoder quantization for the faster 4-bit path. The dropdown also includes `IDM + Klein hybrid`, which first uses IDM-VTON to find the actually changed garment pixels, then uses local Klein LoRA for garment detail inside that IDM delta mask; it writes `hybrid_idm_base.png`, `hybrid_klein_detail.png`, and `hybrid_idm_delta_mask.png` for inspection. `IDM compile + Klein 4-bit pro` combines IDM resident `torch_compile` with the same Klein bnb 4-bit plus Try-On LoRA preset for a faster high-quality hybrid mode after warmup. IDM-VTON can also run through the resident worker, which loads the model once and keeps it in GPU memory between jobs. The engine dropdown includes `flux_redux_catvton`, which sends the backend's dynamic mask, person image, and garment reference through the local ComfyUI Flux Fill + Redux + CatVTON graph on port `8188`; this mode requires the ComfyUI model files listed in `virtual_tryon/docs/comfyui_runpod_usage.md`. Selecting local `klein_lora` stops the resident IDM worker before loading FLUX.2 Klein, moving the unload/load cost to the model switch. Runtime optimization modes can be measured with `virtual_tryon/scripts/benchmark_idm_runtime_modes.py`; IDM TensorRT defaults to the stable VAE-decode path and includes a tested `full_safe` profile for block-wise UNet, UNet-encoder, and VAE decode compilation. Local Klein TensorRT is controlled separately with `TRYON_KLEIN_TRT_PROFILE=vae_decode`.

Try-on masks are anchored to a dynamic body estimate from the uploaded person image, then fall back to a conservative body box when foreground estimation is unreliable or touches a horizontal image edge. Upper-body masks extend lower to cover long shirt hems, reducing leftover source garment at the waist. Adult innerwear categories use a dedicated anatomy-shaped mask path rather than broad outerwear rectangles. `men_underwear` uses a boxer/shorts-coverage mask so old shorts can be removed before generating briefs or boxer briefs; `women_underwear` uses a pelvis/brief-shaped mask with enough dilation to remove the old garment while staying inside the body silhouette. Lower-body and innerwear masks also clear likely side-hand regions before dilation. Innerwear prompts explicitly ask the engine to remove original shorts, pants, briefs, and old fabric patterns inside the target mask before applying the reference garment. Optional innerwear garment-reference cropping can be enabled with `preprocessing.innerwear_reference_crop_enabled`; when enabled it writes `garment_reference_mask.png`, `garment_reference_region.png`, and `garment_engine_input.png`. For Klein LoRA, underwear-bottom prompts explicitly describe adult brief underwear and use the person image as the upper-body preservation reference when no top garment is uploaded, so the bottom reference is not misread as black pants. `women_bra` creates a cup/band/strap-shaped mask. Each job writes `mask_metadata.json`, `mask_innerwear_shape.png`, and, when available, `mask_body_silhouette.png` for debugging. Repeated jobs with the same person image, category, resolution, and mask config reuse cached masks from `data/temp/mask_cache`; mask algorithm changes bump the cache version so old masks are not reused.
