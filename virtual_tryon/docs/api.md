...
- `engine_mode`: optional `idm_vton`, `idm_mask_expanded`, `idm_vton_flux`, `idm_mask_expanded_flux`, `klein_lora`, `klein_bnb_4bit`, `idm_klein_hybrid`, `idm_klein_hybrid_pro`, or `catvton`; default is the configured core engine.
...
ComfyUI workflow files remain available for separate demo/batch reproduction
through the custom node, but ComfyUI is no longer exposed as a production
`engine_mode` in the web API.
